...
int main(int argc, char * argv[]){
    ...
    for(int i = 0; i < sizeof(prefix); i++){ // i = arquivo
        ...
        string output_filename = "normalized_test/";
        ...
        for(int j = 0; j < 1000; j++){
            ...
            float a = std::stof(points[j][8][0]);
            float b = std::stof(points[j][8][1]);
            float c2 = a*a + b*b;
            float c = sqrt(c2);
            float alpha = acos(b/c); // * (180.0 / M_PI); // 0 -> 90 -> 180/180 -> 90 -> 0
            float angle;
            if(a > 0)
                angle = M_PI - alpha;
            else
            ...
            for(int l = 0; l < 20; l++){
                float x = std::stof(points[j][l][0]) / ref_scale;
                float y = std::stof(points[j][l][1]) / ref_scale;
